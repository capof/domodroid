...
public class Graphical_Info_with_achartengine extends FrameLayout implements OnTouchListener, OnLongClickListener {
	...
	public Graphical_Info_with_achartengine(tracerengine Trac,Activity context, int id,int dev_id, String name, 
			...
			int widgetSize, int session_type, final String parameters) throws JSONException {
		super(context);
		this.Tracer = Trac;
		...
		if(with_graph) {
			...
			try {
				...
				json_GraphValues = Rest_com.connect(url+"stats/"+dev_id+"/"+state_key+"/from/"+startTimestamp+"/to/"+currentTimestamp+"/interval/"+step+"/selector/avg");
